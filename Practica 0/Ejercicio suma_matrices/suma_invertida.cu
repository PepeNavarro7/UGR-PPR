...
/* Idéntico a suma_matrices, pero con "x" e "y" invertidas en la asignacion de threads con
la matriz, por lo tanto ahora se hará de manera mucho más ineficiente.
No se consigue la coalescencia */
...
__global__ void MatAdd( float *A, float *B, float *C, int N){
  ...
  int index=x*N+y; // <- AQUI LO INVERTIMOS
  if (x < N && y < N)
...
int main(int argc, char* argv[]){ 
  if (argc == 2){
    ...
    if(TAM_MATRIZ < 46340){
      ...
      // grids de N/16 * N/16 = 625*625 bloques = 390625
      ...
      */
      cout << "Suma INEFICIENTE de 2 matrices de " << TAM_MATRIZ << "*" << TAM_MATRIZ << " elementos." <<endl;
