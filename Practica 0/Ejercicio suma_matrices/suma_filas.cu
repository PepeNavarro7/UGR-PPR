...
/* Ahora cada thread sumará 1 fila entera de la matriz, por lo tanto en lugar de necesitar 
TAM_MATRIZ*TAM_MATRIZ hebras (tantas hebras como elementos), ahora solo necesitaremos 
TAM_MATRIZ hebras (una hebra por fila)*/

__global__ void MatAdd( float *A, float *B, float *C, int N){
    int n_hebra = blockIdx.x * blockDim.x + threadIdx.x;  // calculamos el indice de la hebra
    // la hebra n suma la fila n completa
    C[n_hebra]=0;
    int index;
    if (n_hebra < N && y < N){
    for(int y=0;y<N;++y){ // recorremos toda la fila
        index=n_hebra*N+y; // Busco el indice del elemento de la matriz
...
int main(int argc, char* argv[]){ 
  if (argc == 2){
    ...
    if(TAM_MATRIZ < 46340){
      ...
      cout << "Suma POR FILAS 2 matrices de " << TAM_MATRIZ << "*" << TAM_MATRIZ << " elementos." <<endl;
